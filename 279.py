...
class meshObj():
    ...
    def Update(self, t):
        ...
        for i, v in enumerate(self.mesh.Vertices):
            val = self.pn.noise3(util.Remap(v.X, 10, 90, 0, 1), util.Remap(v.Y, 0, 50, 0, 1), util.Remap(t, 0, 150, 0, 4))
            vec = self.meshOrig.Normals[i]
            vec = rg.Vector3d(vec)
            vec *= val * 20
            xform = rg.Transform.Translation(vec)
        ...
        if True:
            ...
            for j in range(0, len(self.mesh.Faces)-1):
                ...
                if r == rg.PlaneFitResult.Success:
                    ...
                    plane.Origin = center
                    d = rs.VectorDotProduct(plane.Normal, self.mesh.FaceNormals[faceIndex])
                    if d < 0:
                        plane.Flip()
                    
                    projVec = ProjectVectorToPlane(rg.Vector3d(0,-1,0), plane)
                    a = rg.Vector3d.VectorAngle(plane.XAxis, projVec, plane.Normal)
                    plane.Rotate(a, plane.Normal)
                    
                    xform = rg.Transform.PlaneToPlane(rg.Plane.WorldXY, plane)
                    
                    newHead = self.head.Duplicate()
                    xformPlane = rg.Plane(rg.Point3d(.5,.5,0), rg.Vector3d(0,0,1))
                    s = 8
