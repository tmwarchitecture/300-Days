...
class SpecialVertex():
    def __init__(self, system, index):
        ...
        for neighborIndex in self.neighborIndices:
            ...
            self.neighborDistances.append(force.Length)
    ...
    def PullTowardsNeighbor(self):
        for i, neighborIndex in enumerate(self.neighborIndices):
            force = rg.Point3d(self.system.mesh.Vertices[neighborIndex]) - self.pos
            d = force.Length
            val = d - self.neighborDistances[i]
            force.Unitize()
            force *= val *.5
            #self.neighborDistances[i] += force.Length*.1
            self.acc += force
...
class Mesh():
    def __init__(self):
        self.time = time
        offset = 10
        pt0 = rg.Point3d(offset, offset, 30)
        pt1 = rg.Point3d(100-offset, offset, 30)
        pt2 = rg.Point3d(100-offset, 100-offset, 30)
        pt3 = rg.Point3d(offset, 100-offset, 30)
        self.srf = rg.Brep.CreateFromCornerPoints(pt0, pt1, pt2, pt3, sc.doc.ModelAbsoluteTolerance)
        
        param = rg.MeshingParameters()
        param.MaximumEdgeLength = 1
        ...
        self.movingIndex = random.randint(0, self.mesh.Vertices.Count-1)
        self.vec = rg.Vector3d(self.mesh.Normals[self.movingIndex])
        self.vec.Unitize()
        self.vec *= 4
        #self.anchors = []
    ...
    def Update(self, time):
        ...
        if self.time%30 == 0:
            ...
            self.vec = rg.Vector3d(self.mesh.Normals[self.movingIndex])
            self.vec.Unitize()
            self.vec *= random.choice([-4, 4])
            #self.vec = geo.RandomVector3d(2)
        ####
        for vertex in self.vertices:
            if vertex.index == self.movingIndex:
                vertex.ApplyGravity()
            else:
                vertex.PullTowardsNeighbor()
