...
class vObject():
    ...
    def Flatten(self):
        ...
        for neigh in self.neighbors:
            pts.append(rg.Point3d(self.system.mesh.Vertices[neigh]))
        for pt in pts:
            d = (self.vertex-pt).Length
            length += d
        length /= len(pts)
        ...
        if True:
            if len(pts) > 2:
                plane = rg.Plane.FitPlaneToPoints(pts)[1]
                cp = plane.ClosestPoint(self.vertex)
                v = self.vertex-cp
                v.Reverse()
                v *= .5
                self.vel += v
            else:
                for each in self.system.vObjects[self.neighbors[0]].neighbors:
                    if each in self.system.vObjects[self.neighbors[1]].neighbors:
                        sharedNeigh = each
                        break
                if sharedNeigh:
                    pts.append(rg.Point3d(self.system.mesh.Vertices[sharedNeigh]))
                    plane = rg.Plane.FitPlaneToPoints(pts)[1]
                    cp = plane.ClosestPoint(self.vertex)
                    v = self.vertex-cp
                    v.Reverse()
...
class meshObj():
    ...
    def Update(self, t):
        for obj in self.vObjects:
            obj.Flatten()
        for i, obj in enumerate(self.vObjects):
            self.mesh.Vertices[i] += rg.Vector3f(obj.vel.X, obj.vel.Y, obj.vel.Z)
            obj.Flatten()
