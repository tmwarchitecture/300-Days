...
class Ball():
    def __init__(self, system):
        ...
        self.pos = rg.Point3d(random.uniform(10, 90), random.uniform(6, 25), random.uniform(20, 80))
        self.size = 5
...
class Clay():
    def __init__(self, system, attr, interval):
        self.system = system
        self.geo = rg.Box(rg.Plane.WorldXY, rg.Interval(0,100),interval,rg.Interval(0,100))
        self.geo = self.geo.ToBrep()
        self.id = sc.doc.Objects.AddBrep(self.geo, attr)
        self.attr = attr
    
    def Update(self):
        currBrep = self.geo
        for ball in self.system.balls:
            if ball.pos.Y > 25 and ball.pos.Y < 65:
                resultBrep = rg.Brep.CreateBooleanDifference(currBrep, ball.brep, sc.doc.ModelAbsoluteTolerance)
                if len(resultBrep) != 0:
