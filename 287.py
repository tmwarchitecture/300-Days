...
class RainDrop():
    ...
    def Update(self):
        ...
        if mp:
            d = (mp.Point-self.pos).Length
            if d > 5:
                self.alive = False
            n = self.system.mesh.NormalAt(mp)
            p0 = rg.Point3d.Add(mp.Point, n)
            plane = rg.Plane(mp.Point, n)
            p1 = rg.Point3d.Add(p0, rg.Vector3d(0,100,0))
            line = rg.Line(p0,p1)
            x = rg.Intersect.Intersection.LinePlane(line, plane)
            p2 = line.PointAt(x[1])
            if x[0] == False:
                self.alive = False
            self.vel = p2-mp.Point
            self.vel.Unitize()
            self.vel *= 2
...
class meshObj():
    ...
    def InitMesh2(self):
        plane = rg.Plane(rg.Point3d(0,95,100), rg.Vector3d(0,-1,0))
        ...
        scale = 2
        for i, v in enumerate(self.mesh.Vertices):
            val = pn.noise3(util.Remap(v.X, 0, 100, 0, 1*scale), util.Remap(v.Z, 0, 100, 0, 1*scale), 0, 3, 3)
            self.mesh.Vertices[i] = rg.Point3f(v.X, util.Remap(val, -1, 1, 99, 00), v.Z)
    ...
    def InitMesh(self):
        v0 = rg.Point3f(50,50,40)
        v1 = rg.Point3f(54,50,40)
        v2 = rg.Point3f(52,52,40)
        i0 = self.mesh.Vertices.Add(v0)
        i1 = self.mesh.Vertices.Add(v1)
        i2 = self.mesh.Vertices.Add(v2)
        self.mesh.Faces.AddFace(i0,i1,i2)
        
        for i in range(25):
            if False:
                faceIndex = random.randint(0, self.mesh.Faces.Count-1)
                
                vertexList = list(self.mesh.Faces[faceIndex])
                vertices = list(self.mesh.Faces.GetFaceVertices(faceIndex))
                vertices.pop(0)
                removeNum = random.randint(0, 2)
                vertices.pop(removeNum)
                vertexList.pop(removeNum)
            
            #New
            edges = self.mesh.GetNakedEdges()
            
            safety = 0
            while True:
                edgePline = self.mesh.GetNakedEdges()
                segment = random.choice(edgePline[0].GetSegments())
                ...
                v0 = segment.To
                v1 = segment.From
                vn = geo.PointBetweenPoints(v0, v1)
                
                prevVec = v0-v1
                
                perpVec = rg.Vector3d.CrossProduct(prevVec, rg.Vector3d(0,0,1))
                perpVec.Unitize()
                if random.randint(0,1) == 0:
                    perpVec.Reverse()
                perpVec *= 4
                
                newPt = rg.Point3d.Add(vn, perpVec)
                
                if False:
                    self.ids.append(sc.doc.Objects.AddPoint(newPt))
                    self.ids.append(sc.doc.Objects.AddPoint(v1))
                    self.ids.append(sc.doc.Objects.AddPoint(v0))
                    self.UpdateDisplay()
                    print
                    for id in self.ids:
                        sc.doc.Objects.Delete(id, True)
                
                cp = self.mesh.ClosestPoint(newPt)
                d = (rg.Point3d(cp)-rg.Point3d(newPt)).Length
                
                if d > sc.doc.ModelAbsoluteTolerance:
                    i0 = self.mesh.ClosestMeshPoint(v0, 0.0).ComponentIndex.Index
                    i1 = self.mesh.ClosestMeshPoint(v1, 0.0).ComponentIndex.Index
                    i2 = self.mesh.Vertices.Add(newPt)
                    #pline = rg.PolylineCurve([v0, newPt, v1])
                    #sc.doc.Objects.AddCurve(pline)
                    #sc.doc.Views.Redraw()
                    
                    #x = rg.Intersect.Intersection.MeshPolyline(self.mesh, pline)
                    
                    fi = self.mesh.Faces.AddFace(i0, i1, i2)
                    x = self.mesh.Faces.GetClashingFacePairs(5)
                    if len(x)>0:
                        #self.mesh.Faces.DeleteFaces([fi])
                        self.mesh.Vertices.Remove(i2, False)
...
def main():
    ...
    for i in range(numFrames):
        ...
        ################################
        #HUD
        display.UpdateParam1('faces: ' + str(len(mObj.drops)))
