...
class vObject():
    ...
    def UpdateDisplay(self):
        sphere = rg.Sphere(self.system.mesh.Vertices[self.index], 3)
        self.system.ids.append(sc.doc.Objects.AddSphere(sphere))
...
class vSystem():
    ...
    def UpdateDisplay(self):
        ...
        for i, pt in enumerate(self.path):
            ...
            self.attr.ObjectColor = color.GradientOfColors(self.grad, util.Remap(i, 0, len(self.path), 0, 1), 3)
            index = sc.doc.Materials.Add()
            mat = sc.doc.Materials[index]
            mat.DiffuseColor = self.attr.ObjectColor
            mat.CommitChanges()
