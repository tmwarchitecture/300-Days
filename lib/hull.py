...
def set_correct_normal(possible_internal_points,plane): 
    #Make the orientation of Normal correct
...
def checker_plane(a, b): 
    #Check if two planes are equal or not
...
def checker_edge(a, b): 
    # Check if 2 edges have same 2 vertices
    if ((a.pointA == b.pointA)and(a.pointB == b.pointB)) or ((a.pointB == b.pointA)and(a.pointA == b.pointB)):
        return True
    return False

class Edge(): 
    # Make a object of type Edge which have two points denoting the vertices of the edges
...
class Point(): 
    #Point class denoting the points in the space
...
class Plane(): 
    # Plane class having 3 points for a triangle
...
def max_dist_plane_point(plane): 
    # Calculate the maximum distance from the plane
    maxDist = 0
...
def initial(points): 
    # To calculate the extreme points to make the initial simplex
    x_min_temp = 10**9
    x_max_temp = -10**9
    y_min_temp = 10**9
    y_max_temp = -10**9
...
#points = [Point(5,5,5), Point(50,5,5), Point(95,95,0), Point(50,50,50)] # List to store the points
points = [rg.Point3d(5,5,5), rg.Point3d(50,5,5), rg.Point3d(95,95,0), rg.Point3d(50,50,50)] # List to store the points

try:
    extremes = initial2(points) # calculate the extreme points for every axis.
