...
class FaceObj():
    ...
    def AddFaceToMesh(self):
        ...
        if counter == 0 or counter == 2:
            ...
            otherVs = []
            if (v1-line.From).Length < .01 or (v1-line.To).Length < .01:
                #v1 is connected
                otherVs.append(v1)
            else:
                xv = v1
            if (v2-line.From).Length < .01 or (v2-line.To).Length < .01:
                #v2 is connected
                otherVs.append(v2)
            else:
                xv = v2
            if (v3-line.From).Length < .01 or (v3-line.To).Length < .01:
                #v3 is connected
                otherVs.append(v3)
            else:
                xv = v3
...
class meshObj3():
    def __init__(self):
        ...
        #First obj
        plane = rg.Plane(rg.Point3d(60,0,0), rg.Vector3d(1,1,1))
        ...
        self.mesh = rg.Mesh()
        self.mesh = self.mesh.CreateFromBox(self.baseGeo, 1, 1, 1)
        self.mesh.Faces.ConvertQuadsToTriangles()
        self.mesh.Compact()
        brep = self.baseGeo.ToBrep()
        self.wires = brep.GetWireframe(-1)
        
        self.target = geo.Particle(geo.RandomPoint(), geo.RandomVector3d(5))
    ...
    def Update(self, time):
        ...
        #val = util.smoothStart(util.Remap(time, 0, 150, 30, 1))
    ...
    def Subdivide(self, val):
        ...
        for i in range(5):
            ...
            for edgeIndex in range(0,self.mesh.TopologyEdges.Count):
                ...
                d = (p-target).Length
                #print d
                maxDist = util.Remap(d, 41, 100, .5, 60)
                #maxDist = 2
