...
from lib.chull import Vector,Hull  
...
def main():
    ...
    vecObjs = []
    for i in range(10):
        pos = rg.Point3d(random.uniform(0,100),random.uniform(0,100),random.uniform(0,100))
        ptObjs.append(PointObj(pos, geo.RandomVector3d(1)))
        vecObjs.append(Vector(ptObjs[-1].pos.X, ptObjs[-1].pos.Y, ptObjs[-1].pos.Z))
    
    h=Hull(vecObjs)
    for face in h.faces:
        facePts = []
        thisFaceIndices = []
        for eachVertex in face.vertex:
                #v = ptObjs[eachVertex.vnum].pos
                #p = rg.Point3f(v.x, v.y, v.z)
                thisFaceIndices.append(mesh.Vertices.Add(ptObjs[eachVertex.vnum].pos))
        mesh.Faces.AddFace(thisFaceIndices[0], thisFaceIndices[1], thisFaceIndices[2])
    ...
    for i in range(numFrames):
        ...
        for ptObj in ptObjs:
            ptObj.Update()
            ptObj.UpdateDisplay()
            vecObjs.append(Vector(ptObj.pos.X, ptObj.pos.Y, ptObj.pos.Z))
