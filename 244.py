...
class System():
    ...
    def Update(self, time):
        ...
        #    if deadBall in self.balls:
        #        self.balls.remove(deadBall)
        ...
        for particle in self.particles:
            if particle.age > 200:
                ...
                particle.Update()
        for each in particlesToRemove:
            if each in self.particles:
                self.particles.remove(each)
                self.particles.append(Particle(self))
...
class Particle():
    def __init__(self, system):
        self.pos = rg.Point3d(random.uniform(0,100),random.uniform(0,100),random.uniform(0,100))
        self.pts = [self.pos]
        self.vel = rg.Vector3d(random.uniform(-1,1), random.uniform(-1,1), random.uniform(-1,1))
        self.vel.Unitize()
        self.speed = 6
        self.vel *= self.speed
        self.system = system
        pt = self.system.clays[0].geo.ClosestPoint(self.pos, 150)[1]
        self.pos = pt
        self.pts.append(pt)
        #crv = rg.PolylineCurve(self.pts)
        #self.id = sc.doc.Objects.AddCurve(crv)
        self.id = None
        self.size = 3
        self.sphere = rg.Sphere(self.pos, self.size)
        self.brep = self.sphere.ToBrep()
    ...
    def Update(self):
        ...
        #######################################################
        #Get closest pt
        x = self.system.clays[0].geo.ClosestPoint(self.pos, 150)[2]
        face = self.system.clays[0].geo.Faces[x.Index]
        closestPt = self.system.clays[0].geo.ClosestPoint(self.pos, 150)[1]
        
        #Get closest Frame
        r, u, v = face.ClosestPoint(closestPt)
        frame = face.FrameAt(u, v)[1]
        
        #Gravity stuff
        grav = frame.Normal
        grav.Reverse()
        tempVec = rg.Vector3d(self.vel)
        tempVec += grav
        
        #Temp display
        t1 = rg.Point3d.Add(self.pos, grav)
        line = rg.LineCurve(t1, self.pos)
        #self.ids.append(sc.doc.Objects.AddCurve(line))
        #self.normalLines.append(line)
        #######################################################
        
        #Get Future pt on plane
        futurePos = rg.Point3d.Add(self.pos, tempVec)
        #self.ids.append(sc.doc.Objects.AddPoint(futurePos, self.system.attr1))
        
        #Get future frame
        x = self.system.clays[0].geo.ClosestPoint(futurePos, 150)[2]
        futureFace = self.system.clays[0].geo.Faces[x.Index]
        
        #Get closest Frame
        r, u, v = futureFace.ClosestPoint(futurePos)
        futureFrame = futureFace.FrameAt(u, v)[1]
        projPt = futureFrame.ClosestPoint(futurePos)
        #self.ids.append(sc.doc.Objects.AddPoint(projPt, self.system.attr2))
        
        #Get future dir
        #self.ids.append(sc.doc.Objects.AddPoint(closestPt, self.system.attr))
        #self.ids.append(sc.doc.Objects.AddPoint(projPt))
        newVec = projPt-closestPt
        newVec.Unitize()
        newVec *= self.speed
        ...
        #Add Curve
        crv = rg.PolylineCurve(self.pts)
        #self.ids.append(sc.doc.Objects.AddCurve(crv))
        ...
        #loft = rg.Brep.CreateFromLoft(self.normalLines, rg.Point3d.Unset, rg.Point3d.Unset, rg.LoftType.Straight, False)
        #for eachLoft in loft:
        #    self.ids.append(sc.doc.Objects.AddBrep(eachLoft))
...
class Clay():
    def __init__(self, system, color, interval):
        ...
        self.geo = rg.Box(rg.Plane.WorldXY, interval, interval,interval)
