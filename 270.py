...
def Pipe(pts, edge, r):
    p0 = pts[edge.endpts[0].vnum]
    p1 = pts[edge.endpts[1].vnum]
    line = rg.Line(p0, p1)
    midPt = line.PointAt(0)
    frame = rg.Plane(midPt, p0-p1)
    circ = rg.Circle(frame, r)
    cyl = rg.Cylinder(circ, -line.Length)
    cylBrep = cyl.ToBrep(False, False)
    return sc.doc.Objects.AddBrep(cylBrep)

class Blob(geo.Particle):
    def Update(self):
        sc.doc.Objects.Delete(self.id, True)
        self.id = sc.doc.Objects.AddSphere(self.sphere)

class Voxel():
    def __init__(self, system, x,y,z):
        self.system = system
        ...
        self.id = sc.doc.Objects.AddMesh(self.box)
        self.val = None
    
    def Update(self):
        sum = 0
        for target in self.system.targets:
            sum += target.radius/((target.pos - self.pos).Length)
