...
class System():
    ...
    def Update(self, time):
        ...
        planesToRemove = []
        self.voxels.append(Module(self, self.size))
        
        for plane in self.deadPlanes:
            if plane in self.openPlanes:
...
class Pipe():
    def __init__(self, system, pos = None, vec = None):
        self.system = system
        self.majorBrep = rg.Brep()
        self.alive = True
        self.age = 0
        ...
        self.mat.CommitChanges()
        self.attr.MaterialIndex = index
        layer = sc.doc.Layers.FindName("geometry")
        self.attr.LayerIndex = layer.LayerIndex
        ...
        if pos:
            ...
            self.pos, self.dir = self.GetSpawnPointAndVec()
    ...
    def Update(self, time):
        ...
        for plane in self.openPlanes:
            #Rotate base plane
            #plane.Rotate(math.radians(random.uniform(0,360)), plane.Normal)
            plane.Rotate(math.radians(random.uniform(0,4)*90), plane.Normal)
            ...
            if closestPt:
                ...
                while True:
                    ...
                    #vec.Rotate(math.radians(random.uniform(0,2)*30), plane.XAxis)
                    vec.Unitize()
                    vec *= self.length
                    pt2 = rg.Point3d.Add(pt1, vec)
                    if self.age > 5:
    ...
    def GetSpawnPointAndVec(self):
        randPipe = random.choice(self.system.pipes)
        pt = randPipe.segments[-1].path.PointAt(0)
        #pt = randomSeg.path.PointAt(0)
        dir = randPipe.segments[-1].path.FrameAt(0)[1].ZAxis
        #dir = randomSeg.path.FrameAt(0)[1].ZAxis
        return pt, dir
...
class Segment():
    ...
    def UpdateDisplay(self):
        ...
        loft = rg.Brep()
        results = loft.CreateFromLoft([self.crv0, self.crv1], rg.Point3d.Unset, rg.Point3d.Unset, rg.LoftType.Straight, False)
...
class Module():
    def __init__(self, system, size, pos = None):
        ...
        if pos:
            ...
            offset = 0.1
            box = rg.Box(plane, rg.Interval(-self.size/2+offset, self.size/2-offset),rg.Interval(-self.size/2+offset, self.size/2-offset),rg.Interval(offset, self.size-offset))
            self.geo = box.ToBrep()
