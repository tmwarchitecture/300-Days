...
class Ball():
    def __init__(self, system, type):
        ...
        if self.type == 0:
            self.pos = rg.Point3d(random.uniform(10, 90), 6, random.uniform(20, 80))
        elif self.type == 1:
            self.pos = rg.Point3d(random.uniform(10, 90), 94, random.uniform(20, 80))
        self.size = 5
        ...
        self.vel = rg.Vector3d(random.uniform(-1,1),0,random.uniform(-1,1))
        self.vel.Unitize()
        self.vel *= 5
...
class Clay():
    ...
    def Update(self):
        ...
        for tool in self.system.tools:
            ...
            resultBrep = rg.Brep.CreateBooleanDifference(currBrep, tool.brep, sc.doc.ModelAbsoluteTolerance)
            if len(resultBrep) != 0:
                #for result in resultBrep:
                currBrep = resultBrep[0]
                changed = True
        ...
        if changed:
            for i, id in enumerate(self.ids):
                sc.doc.Objects.Delete(id, True)
                self.ids[i] = sc.doc.Objects.AddBrep(self.geo, self.attr)
