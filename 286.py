...
class meshObj():
    def __init__(self):
        ...
        self.InitMesh()
        self.mesh.UnifyNormals()
        #self.mesh = rs.coercemesh(r'e464a55a-3373-483a-ba98-e7d0bde56240')
    ...
    def Update(self, t):
        self.aVel = 1
        self.aVel *= random.uniform(-.1, .1)
        faceIndex = random.randint(0, self.mesh.Faces.Count)
        #deadFace = 0
        faceNeighbors = self.mesh.Faces.AdjacentFaces(faceIndex)
        if len(faceNeighbors) > 0:
            for face in faceNeighbors:
                self.FoldMeshFace(face, faceIndex)
