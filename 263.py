...
class Box():
    ...
    def Update(self):
        ...
        self.edgesPrev = self.boxBrep.GetWireframe(-1)
    ...
    def UpdateDisplay(self):
        self.attr.ObjectColor = color.GradientOfColors(self.grad, util.Remap(self.age, 0, 150, 0, 1))
        self.attr.MaterialSource = rc.DocObjects.ObjectMaterialSource.MaterialFromObject
        index = sc.doc.Materials.Add()
        self.mat = sc.doc.Materials[index]
        self.mat.DiffuseColor = self.attr.ObjectColor
        ...
        if len(self.edgesPrev):
            for i in range(12):
                ...
                loft = rg.Brep.CreateFromLoft([crvA, crvB], rg.Point3d.Unset, rg.Point3d.Unset, rg.LoftType.Straight, False)
                sc.doc.Objects.AddBrep(loft[0], self.attr)
        #edgesPrev = self.prevBrep.GetWireframe(-1)
