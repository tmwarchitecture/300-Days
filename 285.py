...
class meshObj():
    ...
    def InitMesh(self):
        ...
        v1 = rg.Point3f(52,50,40)
        v2 = rg.Point3f(51,51,40)
        ...
        for i in range(5):
            ...
            while True:
                ...
                possiblePts = list(edge)
                
                randInt = random.randint(0, len(possiblePts)-1)
                
                i0 = randInt%(len(possiblePts)-1)
                v0 = possiblePts[i0]
                i1 = (randInt+1)%(len(possiblePts)-1)
                v1 = possiblePts[(randInt+1)%len(possiblePts)]
                
                vn = rg.Point3f((v0.X+v1.X)/2,(v0.Y+v1.Y)/2,(v0.Z+v1.Z)/2 )
    ...
    def Update(self, t):
        ...
        if len(faceNeighbors) > 0:
            neigh = random.choice(faceNeighbors)
            self.FoldMeshFace(faceIndex, neigh)
