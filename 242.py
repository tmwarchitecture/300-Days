...
class Particle():
    ...
    def Update(self):
        ...
        randVec = rg.Vector3d(random.uniform(-1,1), random.uniform(-1,1), random.uniform(-1,1))
        self.vel += randVec
        
        futurePos = rg.Point3d.Add(self.pos, randVec)
        #sc.doc.Objects.AddPoint(futurePos)
        projPt = frame.ClosestPoint(futurePos)
        #sc.doc.Objects.AddPoint(projPt, self.system.attr)
        #sc.doc.Views.Redraw()
        #print
        newVec = closestPt-projPt
        newVec.Unitize()
        newVec *= self.speed
