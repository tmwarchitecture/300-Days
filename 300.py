...
class HUD():
    def __init__(self, skNum, numFrames):
        ...
        self.param1ID = r'c6db3b96-137c-4318-a217-e2430f2ce36b'
...
class EdgeObj():
    def __init__(self, system, i, pt):
        self.system = system
        self.index = i
        self.pt = pt
    
class FaceObj():
    def __init__(self, system, i):
        self.system = system
        self.faceIndex = i
        self.edges = self.system.mesh.TopologyEdges.GetEdgesForFace(i)
    
    def AddFaceToMesh(self):
        counter = 0
        midPts = []
        for edgeIndex in self.edges:
            if self.system.edges[edgeIndex].pt:
                counter += 1
                midPts.append(self.system.edges[edgeIndex].pt)
        #print counter
        
        faceVertices = self.system.mesh.Faces.GetFaceVertices(self.faceIndex)
        ...
        if counter == 0 or counter == 2:
            ...
            ia = self.system.newMesh.Vertices.Add(v1)
            ib = self.system.newMesh.Vertices.Add(v2)
            ic = self.system.newMesh.Vertices.Add(v3)
            
            self.system.newMesh.Faces.AddFace(ia,ib,ic)
            ...
            for edgeIndex in self.edges:
                if self.system.edges[edgeIndex].pt:
                    midPt = self.system.edges[edgeIndex].pt
                    edge = edgeIndex
            
            
            line = self.system.mesh.TopologyEdges.EdgeLine(edge)
            
            line.From
            
            
            v1 = rg.Point3d(faceVertices[1])
            v2 = rg.Point3d(faceVertices[2])
            v3 = rg.Point3d(faceVertices[3])
            
            tol = sc.doc.ModelAbsoluteTolerance
            otherVs = []
            if (v1-line.From).Length < tol or (v1-line.To).Length < tol:
                #v1 is connected
                otherVs.append(v1)
            else:
                xv = v1
            if (v2-line.From).Length < tol or (v2-line.To).Length < tol:
                #v2 is connected
                otherVs.append(v2)
            else:
                xv = v2
            if (v3-line.From).Length < tol or (v3-line.To).Length < tol:
                #v3 is connected
                otherVs.append(v3)
            else:
                xv = v3
            ...
            if len(otherVs) > 1:
                mi = self.system.newMesh.Vertices.Add(midPt)
                i0 = self.system.newMesh.Vertices.Add(otherVs[0])
                i1 = self.system.newMesh.Vertices.Add(otherVs[1])
                xvi = self.system.newMesh.Vertices.Add(xv)
                self.system.newMesh.Faces.AddFace(mi,xvi,i0)
                self.system.newMesh.Faces.AddFace(mi,i1,xvi)
            
        elif counter >= 2:
            
            hasOneChanged = True
            i0 = self.system.newMesh.Vertices.Add(midPts[0])
            i1 = self.system.newMesh.Vertices.Add(midPts[1])
            i2 = self.system.newMesh.Vertices.Add(midPts[2])
            self.system.newMesh.Faces.AddFace(i0,i1,i2)
            ...
            self.system.newMesh.Faces.AddFace(i1,i0,ia)
            self.system.newMesh.Faces.AddFace(i2,i1,ib)
            self.system.newMesh.Faces.AddFace(i0,i2,ic)
    
class TargetObj():
    def __init__(self):
        self.pos = geo.RandomPoint(25,75,25,75,25,75)
        plane = rg.Plane(self.pos, rg.Vector3d(0,0,1))
        xsize = rg.Interval(-25,25)
        ysize = rg.Interval(-25,25)
        zsize = rg.Interval(-25,25)
        box = rg.Box(plane, xsize, ysize, zsize)
        self.brep = box.ToBrep()
        self.vel = geo.RandomVector3d(5)
        self.size = 25
        self.id = None
    def Update(self):
        futurePos = rg.Point3d(self.pos)
        futurePos += self.vel
        move = futurePos-self.pos
        xform = rg.Transform.Translation(move)
        
        if futurePos.X < self.size or futurePos.X > 100-self.size:
            self.vel.X *= -1
        if futurePos.Y < self.size or futurePos.Y > 100-self.size:
            self.vel.Y *= -1
        if futurePos.Z < self.size or futurePos.Z > 100-self.size:
            self.vel.Z *= -1
...
class meshObj3():
    def __init__(self):
        ...
        #Self
        self.brep = None
        self.target1 = TargetObj()
        self.target2 = TargetObj()
    ...
    def MeshBrep(self, brep):
        meshParams = rg.MeshingParameters(100)
        tempMesh = rg.Mesh.CreateFromBrep(brep, meshParams)
    ...
    def Update(self, time):
        ...
        #self.target2.UpdateDisplay()
        
        breps = rg.Brep.CreateBooleanUnion([self.target1.brep, self.target2.brep], sc.doc.ModelAbsoluteTolerance)
        
        for brep in breps:
            self.mesh = self.MeshBrep(brep)
            self.wires = brep.GetWireframe(-1)
            self.Subdivide(util.Remap(time, 0, 150, 30, 1))
    ...
    def Subdivide(self, val):
        ...
        for i in range(5):
            ...
            #Setup Edges
            self.edges = []
            for edgeIndex in range(0,self.mesh.TopologyEdges.Count):
                line = self.mesh.TopologyEdges.EdgeLine(edgeIndex)
                p = line.PointAt(.5)
                target = self.ClosestPtFromEdges(p)
                d = (p-target).Length
                #print d
                maxDist = util.Remap(d, 0, 25, .5, 60)
                #maxDist = 2
                if line.Length < maxDist:
                    p = None
                self.edges.append(EdgeObj(self, edgeIndex, p))
            ...
            for i in range(3):
                ...
                for edge in self.edges:
                    if edge.pt == None:
                        edgeFaces = self.mesh.TopologyEdges.GetConnectedFaces(edge.index)
                        for edgeFace in edgeFaces:
                            edgeFaceEdges = list(self.mesh.TopologyEdges.GetEdgesForFace(edgeFace))
                            if edge.index in edgeFaceEdges:
                                edgeFaceEdges.remove(edge.index)
                            count = 0
                            for edgeFaceEdgeIndex in edgeFaceEdges:
                                if self.edges[edgeFaceEdgeIndex].pt:
                                    count += 1
                            if count == 2:
                                changed = True
                                line = self.mesh.TopologyEdges.EdgeLine(edge.index)
                                edge.pt = line.PointAt(.5)
                if changed == False:
                    #print "Broke out"
                    break
            ...
            self.faces = []
            for i, face in enumerate(self.mesh.Faces):
                self.faces.append(FaceObj(self, i))
                self.faces[-1].AddFaceToMesh()
...
class Link():
    ...
    def Seperate(self, minDist):
        strength = .1
        v = self.objA.pos-self.objB.pos
        if v.Length < minDist:
            delta = minDist - v.Length
            v.Unitize()
            v *= (delta/2)*strength
...
class vObject():
    ...
    def UpdatePosition(self):
        ...
        #self.vel *= util.Constrain(self.vel.Length, 0, 2)
        #if self.isAnchor == False:
        
        self.system.mesh.Vertices[self.index] += rg.Vector3f(self.vel.X, self.vel.Y, self.vel.Z)
        self.pos = rg.Point3d(self.system.mesh.Vertices[self.index])
        self.acc *= 0
        self.vel *= .5
    
    def Grow(self, val):
        v = util.Remap(val, 1, 20, 1, 0)
        #v = .8
        self.vel = self.system.seed.vel * v
        #self.vel = rg.Vector3d(0,0,2)
    
    def UpdateColor(self):
        d = (self.system.target.pos - self.pos).Length
        t = util.Remap(d, 0, 30, 0, 1)
        
        grad = color.GetGradient(10)
        
        col = color.GradientOfColors(grad, t, 3)
        
        self.system.mesh.VertexColors.SetColor(self.index, col)
    
class Seed():
    def __init__(self, system):
        self.system = system
        self.pos = rg.Point3d(50,50,10)
        self.id = None
        self.growthPts = [self.pos]
        self.vel = None
    
    def Grow(self, t):
        #vec = rg.Vector3d(random.uniform(-1,1), random.uniform(-1,1), random.uniform(.1,1))
        #vec.Unitize()
        #newPt = rg.Point3d.Add(self.growthPts[-1], vec)
        #self.growthPts.append(newPt)
        
        vec = rg.Vector3d(random.uniform(-1,1), random.uniform(-1,1), random.uniform(.1,1))
        vec.Unitize()
        prevPos = rg.Point3d(self.pos)
        self.pos += vec
...
class meshObj4():
    def __init__(self):
        ...
        self.mesh = rs.coercemesh(r'941e5133-6d8f-4a2a-853f-f4d3588c3b95')
        ...
        if False:
            self.links = []
            for i in range(len(self.mesh.Vertices)):
                for j in range(i+1, len(self.mesh.Vertices)):
                    if (self.vertices[i].pos - self.vertices[j].pos).Length < 30:
                        self.links.append(Link(self.vertices[i], self.vertices[j]))
    
    def Update(self, t):
        if t%20 == 0:
            val = random.randint(0, len(self.vertices)-1)
            self.target = self.vertices[val]
        
        
        for i, v in enumerate(self.vertices):
            self.MoveVertexAlongNormal(i)
        
        #for i, v in enumerate(self.mesh.Vertices):
        #    col = drawing.Color.FromArgb(random.randint(0,255), random.randint(0,255), random.randint(0,255))
        #    self.mesh.VertexColors.SetColor(i, col)
        
        if False:
            indices = []
            for i in range(0, self.mesh.TopologyEdges.Count):
                line = self.mesh.TopologyEdges.EdgeLine(i)
                if line.Length > 5:
                    indices.append(i)
            for i in indices:
                faces = self.mesh.TopologyEdges.GetConnectedFaces(i)
                self.mesh.Faces.DeleteFaces(faces)
    ...
    def UpdateDivisions(self):
        #Split line that are too long
        for edgeIndex in range(0, self.mesh.TopologyEdges.Count):
            line = self.mesh.TopologyEdges.EdgeLine(edgeIndex)
            d = line.Length
            if d > 13:
                x = self.mesh.TopologyEdges.SplitEdge(edgeIndex, .5)
                #v = self.mesh.Vertices[self.mesh.Vertices.Count-1]
                #self.vertices.append(vObject(self, v, self.mesh.Vertices.Count-1))
                #indices = self.mesh.TopologyEdges.GetTopologyVertices(edgeIndex)
                #v0 = self.vertices[indices[0]].vel
                #v1 = self.vertices[indices[1]].vel
                #newVec = rg.Vector3d(0,0,0)
                #newVec = (v0+v1)*.5
                #self.vertices[-1].vel = newVec
    
    def InitVertices(self):
        #Setup vertices
        for i, v in enumerate(self.mesh.Vertices):
            self.vertices.append(vObject(self, v, i))
    ...
    def MoveVertexAlongNormal(self, i):
        vertex = self.mesh.Vertices[i]
        
        v = rg.Point3d(vertex)
        
        d = (self.target.pos - v).Length
...
def main():
    ...
    mObj = meshObj4()
    #for i in range(1):
    #    mObj.UpdateDivisions()
    
    #mObj.SubdivideTriangle()
    ...
    for i in range(numFrames):
        ...
        ################################
        #HUD
        display.UpdateParam1('vertices: ' + str(len(mObj.vertices)))
