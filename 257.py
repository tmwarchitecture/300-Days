...
class Mesh():
    ...
    def Update(self):
        ...
        #self.meshDisplay.Vertices
        index, targetPt = self.mesh.ClosestPoint(self.target.pos, 100)
        #target = self.mesh.Vertices[index]
        for i in range(self.mesh.Vertices.Count):
            d = rs.Distance(targetPt, self.mesh.Vertices[i])
    ...
    def MeshWindow(self):
        ...
        for i in range(self.meshDisplay.Faces.Count):
            ...
            offset = util.Remap(z, 30, 60, 0.33, .00)
        ...
        self.meshDisplay.Faces.DeleteFaces(range(self.mesh.Faces.Count))
